...
>
> The project's own test suite is **157 passing across three apps** (47 · 46 · 64). Those are separate: run
...
├── sentinel/                  ⭐ MCP-2 — drift and the refusal · 3 tools, 47 tests
│   ├── src/modules/mentor/       explain_drift, withhold_fix, mentor_status
│   └── src/widgets/              the causal-timeline UI
├── mcp-profile/               ⭐ MCP-3 — the student record and the flashcards · 9 tools, 64 tests
...
The three deployed servers offer 20 tools between them (8 · 3 · 9) and every one is MENTOR's
...
`npm run verify` should end with `47/47 pass` and the fixture guard's `ok` lines. **It needs only
Node** — no Python, no network, no key.

⚠️ **It does not run the whole suite.** The root scripts were written when `sentinel/` was the
only app and still only reach it, so 110 of the 157 tests never run from the root
(`GAPS.md` Gap 18). Until that is fixed, the honest health check is three commands:

```bash
(cd mcp-roster && npm test)   # 46
(cd sentinel   && npm test)   # 47
(cd mcp-profile && npm test)  # 64
```
...
| Whole suite | ✅ **157** (47 · 46 · 64), offline, no API key, no model |
